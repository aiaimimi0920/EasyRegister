...
def _mailbox_provider_consecutive_blacklist_failure_total(failure_reasons: Any) -> int:
    return _mailbox_failure_reason_total(
        failure_reasons,
        PROVIDER_CONSECUTIVE_BLACKLIST_FAILURE_REASONS,
    )
...
def _mailbox_provider_dynamic_blacklist_recovery_qualified(stats: dict[str, Any]) -> bool:
    ...
    if provider_consecutive_threshold > 0:
        ...
        if (
            ...
            and _mailbox_provider_consecutive_blacklist_failure_total(failure_reasons) >= provider_consecutive_threshold
        ):
            return False
    ...
    if provider_otp_threshold > 0:
        try:
            ...
            consecutive_failures = 0
        if (
            consecutive_failures >= provider_otp_threshold
            and _mailbox_failure_reason_total(failure_reasons, EMAIL_OTP_FAILURE_REASONS) >= provider_otp_threshold
        ):
            return False
    try:
...
def _mailbox_provider_consecutive_failures_reaches_blacklist_threshold(stats: dict[str, Any]) -> bool:
    ...
    return (
        ...
        and _mailbox_provider_consecutive_blacklist_failure_total(stats.get("failureReasons")) >= threshold
    )
...
def _mailbox_avoid_payload(
    *,
    ...
    provider_keys = list(
        _normalize_mailbox_avoid_values(
            excluded_provider_type_keys,
            kind="provider",
        )
    )
    domains = list(_normalize_mailbox_avoid_values(excluded_domains, kind="domain"))
    ...
    if provider_keys:
        payload["providerTypeKeys"] = provider_keys
    if domains:
        payload["domains"] = domains
    if email_addresses:
        payload["emailAddresses"] = email_addresses
    reason = str(avoid_reason or "").strip()
    if reason:
...
def resolve_mailbox(
    *,
    ...
    if normalized_preallocated_email and recreate_preallocated_email:
        ...
        recreate_excluded_email_addresses = _resolve_mailbox_excluded_email_addresses(
            avoid_emails=avoid_emails,
        )
        recreate_avoid = _mailbox_avoid_payload(
            excluded_email_addresses=recreate_excluded_email_addresses,
            avoid_reason=avoid_reason,
        )
        try:
    ...
    plan_avoid = _mailbox_avoid_payload(
    ...
    try:
        ...
        if selected_domain:
            ...
            selected_excluded_email_addresses = _resolve_mailbox_excluded_email_addresses(
                avoid_emails=avoid_emails,
            )
            selected_avoid = _mailbox_avoid_payload(
                excluded_domains=selected_excluded_domains,
                excluded_email_addresses=selected_excluded_email_addresses,
                avoid_reason=avoid_reason,
            )
            json_log(
                {
