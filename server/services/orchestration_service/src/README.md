...
这个目录现在只保留 `RegisterService` 的顶层 DST 编排与调度适配层。
...
当前运行时不再在 `RegisterService` 本地做 provider 顺序路由。
...
但 `RegisterService` 仍然只调用 `EasyProtocol`，并默认使用 `strategy`
模式由它决定当前使用哪个可用执行器。
...
这些协议语义步骤不再在 `RegisterService` 内部本地执行，而是交给 `ProtocolService`
...
`RegisterService` 自身的容器实例编排也已经独立出来，不再放在 `deploy/` 下：

- `C:\Users\Public\nas_home\AI\GameEditor\RegisterService\compose\docker-compose.yaml`

当前容器入口 [infinite_runner.py](C:\Users\Public\nas_home\AI\GameEditor\RegisterService\server\services\orchestration_service\src\infinite_runner.py)
